...
def flip_random_bits(s: str) -> str:
    """
    基于 AFL 变异算法策略中的 bitflip 实现相邻 N 位翻转（N = 1, 2, 4），其中 N 为随机生成
...
def arithmetic_random_bytes(s: str) -> str:
    """
    基于 AFL 变异算法策略中的 arithmetic inc/dec 实现相邻 N 字节随机增减（N = 1, 2, 4），其中 N 为随机生成
    字节随机增减：
...
def interesting_random_bytes(s: str) -> str:
    """
    基于 AFL 变异算法策略中的 interesting values 实现相邻 N 字节随机替换为 interesting_value（N = 1, 2, 4），其中 N 为随机生成
